"""
author: Tiffany Lee
file: /Users/tiffanylee/Downloads/Personal Projects/Advent_of_Code/2022/InputFiles_2022/Day08_Input.txt
...
0 is the shortest and 9 is the tallest. A tree is visible if all of the
other trees between it and an edge of the grid are shorter than it.
Only consider trees in the same row or column; that is, only look
...
(It isn't visible from the right or bottom since other trees of height
5 are in the way.)
...
edge, at least one of its viewing distances will be zero.) The Elves don't
care about distant trees taller than those found by the rules above; the
proposed tree house has large eaves to keep it dry, so they wouldn't be able to
...
def part1(grid):
    ...
    for r in range(len(grid)):
        for c in range(len(grid[r])):
            currTree = grid[r][c]
            if all(currTree > grid[r][x] for x in range(c)) or \
                all(currTree > grid[r][x] for x in range(c+1, len(grid[r]))) or \
                all(currTree > grid[x][c] for x in range(r)) or\
                all(currTree > grid[x][c] for x in range(r+1, len(grid))):
                visible += 1
    print("Part One: Consider your map; "
          + "how many trees are visible from outside the grid?"
          + "\nANSWER: " + str(visible) + " trees")
...
def main():
    ...
    grid = [[int(line[i]) for i in range(len(line))] for line in info]
    part1(grid)
    part2(grid)
main()