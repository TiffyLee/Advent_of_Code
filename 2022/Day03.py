"""
author: Tiffany Lee
file: /Users/tiffanylee/Downloads/Personal Projects/Advent_of_Code/2022/InputFiles_2022/Day03_Input.txt
...
rucksack. The Elves have made a list of all of the items currently in
each rucksack (your puzzle input), but they need your help finding the
errors. Every item type is identified by a single lowercase or uppercase
letter (that is, a and A refer to different types of items). The list of
items for each rucksack is given as characters all on a single line.
...
in the first compartment, while the second half of the characters
...
and rsFMfFZSrLrFZsSL. The only item type that appears in
...
with another issue. For safety, the Elves are divided into groups of three.

Every Elf carries a badge that identifies their group. For efficiency,
within each group of three Elves, the badge is the only item type carried
by all three Elves. That is, if a group's badge is item type B, then all
three Elves will have item type B somewhere in their rucksack, and
...
sticker attachment efforts: here, they are 18 (r) for the first group
...
def part1(filename):
    ...
    for line in open(filename):
        ...
        if "a" <= item <= "z":
            sumP += ord(item) - ord("a") + 1
        else:
            sumP += ord(item) - ord("A") + 27
...
def part2(filename):
    allGroups = [group.strip() for group in open(filename)]
    i = 0
    sumP = 0
    while i < len(allGroups):
        groups = [ allGroups[i], allGroups[i+1], allGroups[i+2] ]
        item, = set.intersection(*map(set, groups))
        if "a" <= item <= "z":
            sumP += ord(item) - ord("a") + 1
            i += 3
        else:
            sumP += ord(item) - ord("A") + 27
            i += 3
    print("\nPart Two: Find the item type that corresponds to the badges of each three-Elf group."
          + "What is the sum of the priorities of those item types?"
          + "\nANSWER: " + str(sumP))

def main():
    filepath = "/Users/tiffanylee/Downloads/Personal Projects/Advent_of_Code/2022/InputFiles_2022/Day03_Input.txt"
    part1(filepath)
    part2(filepath)
main()