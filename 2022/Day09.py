"""
author: Tiffany Lee
file: /Users/tiffanylee/Downloads/Personal Projects/Advent_of_Code/2022/InputFiles_2022/Day09_Input.txt

--- Day 9: Rope Bridge ---
This rope bridge creaks as you walk along it. You aren't sure how old it is, or
whether it can even support your weight. It seems to support the Elves just fine,
though. The bridge spans a gorge which was carved out by the massive river far
below you. You step carefully; as you do, the ropes stretch and twist. You decide
to distract yourself by modeling rope physics; maybe you can even figure out
where not to step.
...
toward the head. Due to nebulous reasoning involving Planck lengths, you should
...
you can determine how the tail will move. Due to the aforementioned Planck lengths,
the rope must be quite short; in fact, the head (H) and tail (T) must always be
...
adjacent to the tail. Visually, these motions occur as follows
...
at least once. In this diagram, s again marks the starting position
...
toward you as you fall through the air! The ropes are moving too quickly to
grab; you only have a few seconds to choose how to arch your body to avoid
being hit. Fortunately, your simulation can be extended to support longer
ropes.
...
using the same rules as before. Using the same series of motions as the
above example, but with the knots marked H, 1, 2, ..., 9, the motions now
occur as follows:
...
"""
def part1(info, moveRow, moveCol):
    visited = set()
    H = [0, 0]
    T = [0, 0]
    for line in info:
        ...
        y = int(y)
        for _ in range(y):
            H = (H[0]+moveRow[x], H[1]+moveCol[x])
            newX = (H[0]-T[0])
            newY = (H[1]-T[1])
            if abs(newX) > 1 or abs(newY) > 1:
                if newX == 0:
                    T[1] += newY // 2
                elif newY == 0:
                    T[0] += newX // 2
                else:
                    T[0] += 1 if newX > 0 else -1
                    T[1] += 1 if newY > 0 else -1
            visited.add(tuple(T))
    print("Part One: Simulate your complete hypothetical series of motions. "
          + "How many positions does the tail of the rope visit at least once?"
          + "\nANSWER: " + str(len(visited)) + " positions")

def part2(info, moveRow, moveCol):
    visited = set()
    ...
    print("\nPart Two: Simulate your complete series of motions on a larger "
          + "rope with ten knots. How many positions does the tail of the "
          + "rope visit at least once?"
          + "\nANSWER: " + str(len(visited)) + " positions")

def main():
    filepath = "/Users/tiffanylee/Downloads/Personal Projects/Advent_of_Code/2022/InputFiles_2022/Day09_Input.txt"
    info = [line.strip() for line in open(filepath)]
    moveRow = {"U": -1, "D": 1, "L": 0, "R": 0}
    moveCol = {"U": 0, "D": 0, "L": -1, "R": 1}
    part1(info, moveRow, moveCol)
    part2(info, moveRow, moveCol)
main()