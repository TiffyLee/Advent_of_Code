"""
author: Tiffany Lee
file: /Users/tiffanylee/Downloads/Personal Projects/Advent_of_Code/2022/InputFiles_2022/Day07_Input.txt
...
various sizes. Since the disk is full, your first step should probably be to
find directories that are good candidates for deletion. To do this, you need to
determine the total size of each directory. The total size of a directory is the
...
--- Part Two ---
Now, you're ready to choose a directory to delete. The total disk space available
to the filesystem is 70000000. To run the update, you need unused space of at
least 30000000. You need to find a directory you can delete that will free up
...
"""
def main():
    filepath = "/Users/tiffanylee/Downloads/Personal Projects/Advent_of_Code/2022/InputFiles_2022/Day07_Input.txt"
    ...
    for key, value in dirSizes.items():
        if value <= 100000:
            ...
            dirSum += value
        if value >= freeUp:
            sizeDel = min(sizeDel, value)
            dirDel = key
    print("Part One: Find all of the directories with a total size of at most "
          + "100000. What is the sum of the total sizes of those directories?"
          + "\nANSWER: The size of following directories all sum to " + str(dirSum)
          , end="\n\t")
    print(*dirName, sep="\n\t")
    print("\nPart Two: Find the smallest directory that, if deleted, would free "
          + "up enough space on the filesystem to run the update. What is the "
          + "total size of that directory?"
          + "\nANSWER: " + dirDel + " which has a size of " + str(sizeDel))
main()