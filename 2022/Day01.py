...
the fruit grows. To supply enough magical energy, the expedition needs to
retrieve a minimum of fifty stars by December 25th. Although the Elves assure
you that the grove has plenty of fruit, you decide to grab any fruit you see
along the way, just in case. Collect stars by solving puzzles.
...
out of snacks. To avoid this unacceptable situation, the Elves would instead
like to know the total Calories carried by the top three Elves carrying
...
then the fifth Elf (with 10000 Calories). The sum of the Calories carried
...
def main():
    ...
    for elf in ("\n".join(info).split("\n\n")):
        ...
        for cal in elf.split("\n"):
            elfCal += int(cal)
        calories.append(elfCal)

    unsortedCals = calories
    calories = sorted(calories)

    print("Part One: Find the Elf carrying the most Calories. "
          + "How many total Calories is that Elf carrying? "
          + "\nANSWER: Elf #" + str(unsortedCals.index(calories[-1])+1)
          + " with " + str(calories[-1]) + " Calories")

    print("\nPart Two: Find the top three Elves carrying the most Calories. "
          + "How many Calories are those Elves carrying in total?"
          + "\nANSWER: Elves #" + str(unsortedCals.index(calories[-1])+1)
          + ", #" + str(unsortedCals.index(calories[-2])+1)
          + ", and #" + str(unsortedCals.index(calories[-3]) + 1)
          + " with a total of " + str(calories[-1]+calories[-2]+calories[-3])
          + " Calories")
main()